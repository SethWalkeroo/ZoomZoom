...
class ZoomZoom:
    ...
    def meeting_link(self, data):
        while True:
            ...
            print()
            with indent(4, quote=' *'):
                puts(colored.yellow('WARNING: make you disable any window tiling before running this script or else it will not work.'))
                puts(colored.yellow('WARNING: also make sure you download and add a compatible webdriver for your browser and put it in the "webdriver" directory.'))
            print()
            with indent(4):
    ...
    def save_meeting(self, meeting_info, data):
        if not meeting_info[2]:
            ...
            meeting_data = data
            print()
            save_meeting = input('Would you like to save this meeting for future use? [y/n]: ')
            if save_meeting == 'y':
                print()
                meeting_name = input('Enter name for saved meeting: ')
                meeting_data["meetings"].update({meeting_name:{"id":0, "psw":0}})
    ...
    def automatic_join(self, meeting_info):
        ...
        meeting_psw = meeting_info[1]
        browser = webdriver.Chrome(options=self.chrome_options, executable_path='webdriver/chromedriver')
        browser.get('https://zoom.us/')
        join_meeting = browser.find_element_by_xpath('//*[@id="btnJoinMeeting"]')
        join_meeting.click()
        sleep(1)
